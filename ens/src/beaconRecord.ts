// Pure logic only — no network, no viem client, no `node:fs`. Split out of
// beacon.ts so the frontend (which runs in a browser and cannot bundle
// `node:fs`) can import the record type and the "why is this blocked"
// explanation without pulling in the Hardhat-artifact loader that the
// scripts and tests use on the Node side. beacon.ts re-exports everything
// here, so existing imports of it are unaffected.
